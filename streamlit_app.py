...
def rewrite_and_correct_query(user_raw_input: str, openai_client) -> str:
    ...
    """
    system_prompt = """
    אתה רכיב טיוב שאילתות מתקדם עבור מנוע חיפוש נהלים של משאבי אנוש (HR) בחברת "בזק".
    תפקידך לנתח את קלט המשתמש, לזהות שגיאות כתיב קשות, שגיאות פונטיות וסלנג, ולהמיר אותם לשאילתת חיפוש מקצועית וברורה בעברית.
    
    דגשים קריטיים:
    1. שגיאות פונטיות: שים לב במיוחד למילים שנכתבות שגוי אך נשמעות כמו מונחי HR (למשל: "השל" = "אש"ל", "הבאה" = "הבראה", "מגיא" = "מגיע", "חלת" = "חל"ת").
    2. אל תענה על השאלה! תפקידך הוא רק לנסח את השאלה מחדש בצורה תקינה.
    
    דוגמאות לטיוב (Few-Shot Examples):
    קלט: "כמה ימי השל מגיא לי?"
    פלט: "כמה ימי אש"ל מגיעים לי?"
    
    קלט: "מתי אני מקבל דמי הבאה"
    פלט: "מתי משלמים דמי הבראה?"
    
    קלט: "איך יוצאים לחלת"
    פלט: "מה הנהלים ליציאה לחופשה ללא תשלום (חל"ת)?"
    
    החזר אך ורק את השאילתה המתוקנת בשורה אחת בלבד, ללא שום טקסט נוסף או מרכאות.
    """
    try:
